...
def parse_model(d, ch, verbose=True):
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in base_modules:
            ...
            c1 = ch[f]
            original_out_planes = args[1] if len(args) >= 2 else c1
            if original_out_planes != nc:
                c2 = make_divisible(min(original_out_planes, max_channels) * width, 8)
            else:
                c2 = original_out_planes
            args = [c1, c2, args[2], args[3]] if len(args) >= 4 else [c1, c2]
